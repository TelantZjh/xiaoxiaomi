<template>
  <div class="about">
   ...
   <!--  <h3 v-for="(item,index) in movies" :key="index">
    ...
    <h1 v-for="(item) in zjh" :key="item.courseId">{{ item.coverTitle }}</h1>
...
export default {
  ...
  methods:{
    getList(){
      this.$http.get("http://localhost:8080//recommend/getRandContent?_t=1584929000172&csrfToken=&type=3&pageSize=10")
      .then(res=>{
        this.zjh = res.data.data.data;
      })
    },
    getFeiyan(){
